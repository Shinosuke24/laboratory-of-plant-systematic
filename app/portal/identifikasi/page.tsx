...
export default function IdentifikasiPage() {
  const [items, setItems] = useState<Identifikasi[]>([]);
  ...
  const fetchIdentifikasi = async () => {
    try {
      ...
      if (!response.ok) {
        setItems([]);
        setErrorMessage(data?.error || "Failed to fetch identification data");
        return;
      }
      ...
      setItems([]);
      setErrorMessage("Failed to fetch identification data");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setErrorMessage(
          payload?.error || "Failed to submit identification request",
        );
      }
    } catch (error) {
      console.error("Error submitting:", error);
      setErrorMessage("Failed to submit identification request");
    } finally {
        ...
        <div className="space-y-6 sm:space-y-8">
          ...
          <div className="flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-center">
            <div>
              ...
              <p className="mt-1 text-sm text-gray-600 sm:mt-2 sm:text-base">
                Complete the identification request details based on
                requirements
              </p>
            </div>
            <Button
          ...
          {showForm && (
            <Card className="border-green-200">
              <CardHeader>
                <CardTitle>Plant Identification Request Form</CardTitle>
                <CardDescription>
                  Fill in the fields below before submitting to admin
                </CardDescription>
              </CardHeader>
              <CardContent>
                <form onSubmit={handleSubmit} className="space-y-4">
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-900 mb-1">
                      Research Schedule (day and date) *
                    </label>
                    <Textarea
                      required
                      ...
                      onChange={(e) =>
                        setFormData({
                          ...formData,
                          jadwalPelaksanaan: e.target.value,
                        })
                      }
                      placeholder="Example: Monday, May 12, 2026"
                      rows={3}
                    />
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      ...
                      <Input
                        required
                        value={formData.namaKode}
                        onChange={(e) =>
                          setFormData({ ...formData, namaKode: e.target.value })
                        }
                        placeholder="Example: KDS-PLT-001"
                      />
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-900 mb-1">
                      ...
                      <Input
                        required
                        value={formData.familyKelompok}
                        onChange={(e) =>
                          setFormData({
                            ...formData,
                            familyKelompok: e.target.value,
                          })
                        }
                        placeholder="Example: Asteraceae"
                      />
                    </div>
                  </div>
          ...
          <div>
            <h2 className="text-2xl font-bold text-gray-900 mb-4">
              Identification Submission History
            </h2>
            {errorMessage && (
            ...
            {loading ? (
              ...
              <div className="text-center py-12">
                <p className="text-gray-600 mb-4">
                  No identification submissions yet
                </p>
                <Button
                    ...
                    <CardContent className="pt-6">
                      <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                        <div className="flex-1">
                          <h3 className="text-lg font-semibold text-gray-900">
                            {item.namaKode || "Sample Code"}
                          </h3>
                          <p className="text-sm text-gray-600">
                            Family/Group: {item.familyKelompok || "-"}
                          </p>
                          <p className="text-sm text-gray-600">
                            Name: {item.nama || "-"}
                          </p>
                      ...
                      {item.jadwalPelaksanaan && (
                        <p className="text-gray-600 mb-3">
                          Schedule: {item.jadwalPelaksanaan}
                        </p>
                      )}

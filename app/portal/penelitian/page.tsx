...
export default function PenelitianPage() {
  const [items, setItems] = useState<Penelitian[]>([]);
  ...
  const fetchPenelitian = async () => {
    try {
      ...
      if (!response.ok) {
        setItems([]);
        setErrorMessage(data?.error || "Failed to fetch research request data");
        return;
      }
      ...
      setItems([]);
      setErrorMessage("Failed to fetch research request data");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!suratPermohonanUrl) {
        setErrorMessage("A research request letter PDF is required");
        return;
      }
      ...
      if (response.ok) {
        ...
        setErrorMessage(payload?.error || "Failed to submit research request");
      }
    } catch (error) {
      console.error("Error submitting:", error);
      setErrorMessage("Failed to submit research request");
    } finally {
          ...
          <div className="flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-center">
            <div>
              ...
              <p className="text-gray-600 mt-2">
                Complete the research request form based on the requirements
              </p>
            </div>
            <Button
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  ...
                  <div>
                    ...
                    <Textarea
                      ...
                      placeholder="Write the planned research day and date"
                      rows={3}
                    />
                  </div>
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-900 mb-1">
                      Upload Research Request Letter (PDF) *
                    </label>
                    <Input
                      required
                      type="file"
                  ...
                  <div className="flex flex-col gap-2 pt-4 sm:flex-row">
                    ...
                    >
                      {isUploadingPdf ? "Uploading PDF..." : "Submit Request"}
                    </Button>
                    <Button
                      type="button"
                      variant="outline"
                      ...
                      <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                        <div className="flex-1">
                          <h3 className="text-lg font-semibold text-gray-900">
                            {item.nama || "Research Request"}
                          </h3>
                          <p className="text-sm text-gray-600">
                            NIM: {item.nim || "-"}
                          </p>
                          <p className="text-sm text-gray-600">
                            Institution: {item.namaInstitusi || "-"}
                          </p>
                        </div>

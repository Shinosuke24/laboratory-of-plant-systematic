...
export async function GET(
  ...
  try {
    ...
    if (penelitian.userId !== user.id && !isAdmin) {
...
export async function PUT(
  ...
  try {
    ...
    const existing = await prisma.penelitian.findUnique({
      where: { id },
      include: { user: true },
    });
    ...
    if (
      ...
      try {
        await sendApprovalStatusEmail({
          ...
          serviceName: "Penelitian",
          status: String(body.status),
          submissionTitle: existing.nama || existing.title,
        });
      } catch (mailError) {
        console.error(
          "[EMAIL] Failed to send research notification:",
          mailError,
        );
      }
    }
    ...
    console.error("[API] Error updating penelitian:", error);
...
export async function DELETE(
  ...
  try {
    ...
    console.error("[API] Error deleting penelitian:", error);
